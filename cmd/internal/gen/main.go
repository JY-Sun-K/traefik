...
const rootPkg = "traefik/v3/pkg/config/dynamic"
...
func run(dest string) error {
	...
	centrifuge.ExcludedFiles = []string{
		"traefik/v3/pkg/types/logs.go",
		"traefik/v3/pkg/types/metrics.go",
	}
...
func cleanType(typ types.Type, base string) string {
	if typ.String() == "traefik/v3/pkg/tls.FileOrContent" {
		return "string"
	}
	...
	if strings.Contains(typ.String(), "traefik/v3/pkg/") {
		return strings.ReplaceAll(typ.String(), "traefik/v3/pkg/", "")
	}
...
func cleanPackage(src string) string {
	...
	case "github.com/traefik/paerser/types":
		return ""
	case "traefik/v3/pkg/tls":
		return path.Join(destModuleName, destPkg, "tls")
	case "traefik/v3/pkg/types":
